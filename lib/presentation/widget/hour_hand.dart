...
import 'package:analog_clock_test/presentation/painter/hour_hand_painter.dart';
...
class HourHand extends StatefulWidget {
  ...
  const HourHand({super.key, required this.minute, required this.hour});
...
class _HourHandState extends State<HourHand>
  ...
  _panUpdateHandler(DragUpdateDetails d) {
    ...
    setState(() {
      ...
      double degrees = roundToBase(a.roundToDouble(), 10);
      _valueChoose = degrees ~/ 30 == 12 ? 0 : degrees ~/ 30;
      BlocProvider.of<ClockBloc>(context).add(SetHourEvent(hour: _valueChoose));
    });
  }
